...
document.addEventListener("DOMContentLoaded", function () {
  ...
  customDropdown.querySelectorAll(".options li").forEach(function (option) {
    option.addEventListener("click", function () {
      const selectedValue = this.getAttribute("data-value");
      const selectedText = this.textContent;

      if (selectedLanguages.textContent.includes(selectedText)) return;

      const selectedListItem = document.createElement("div");
      selectedListItem.textContent = selectedText;
      selectedLanguages.appendChild(selectedListItem);

      if (selectedOptions.textContent === "Select languages") {
  ...
  document.addEventListener("click", function (e) {
    if (!customDropdown.contains(e.target)) {
      customDropdown.querySelector(".options").style.display = "none";
    }
  });
});
